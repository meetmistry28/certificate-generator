...
const generatePDFService = async (
    ...
    // Draw a border around the entire page without overlapping "ENGINEER REMARKS"
    doc.lineWidth(2)
        .strokeColor('#000')
        .rect(margin, margin, doc.page.width - 2 * margin, pageHeight - 2 * margin)
        .stroke();
    ...
    doc.rect(margin, margin, doc.page.width - 2 * margin, pageHeight - 2 * margin).stroke();
    doc.end();
    return fileName;
};
